...
Math{ 
    ...
    Number_of_threads = 8
    NewtonPlot(
    MinError Residual Update Error
    )
}
...
Solve{
  ...
  Quasistationary ( 
    InitialStep= 1e-2 Increment= 1.41 Decrement=2.0
    ...
    NewCurrentFile="IcVc_"
  Quasistationary ( 
    InitialStep= 1e-2 Increment= 1.41 Decrement=2.0
    MinStep= 1e-7     MaxStep= 0.02
    Goal { Name="Collector" Voltage= 20.0 }
  ){
    Coupled { 
     ...
     CurrentPlot(Time=(
   	Range=(0.0 1.0) Intervals=20;
    	Range=(0.0 0.2) Intervals=20)
    )
    #Plot(FilePrefix="Profile_n@node@" Time=(Range=(0 1.0) Intervals=50) noOverwrite)
  }
}